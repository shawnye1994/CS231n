...
def image_from_url(url):
  ...
  """
  #when I run the original code, encounted a problem that can't delete the temp file
  # so I changed two lines of the code
  try:
    f = urllib2.urlopen(url)
    #_, fname = tempfile.mkstemp()     the original code
    ftest,fname = tempfile.mkstemp()
    with open(fname, 'wb') as ff:
      ff.write(f.read())
    img = imread(fname)
    os.close(ftest)  #must close before remove
    os.remove(fname)
    return img
