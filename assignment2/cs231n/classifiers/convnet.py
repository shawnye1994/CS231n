...
def five_layer_convnet(X, model, y=None, reg=0.0):
    #[conv-relu-pool]x2- conv - relu - [affine]x2 - softmax ,use L2 regularization
    #where the conv layer uses stride-1 "same" convolutions to preserve the input size;the pool layer uses non-overlapping
    #2x2 pooling regions. We use L2 regularization on both the convolutional layer weights and the affine layer weights.
    
    W1,b1,W2,b2 = model['W1'],model['b1'],model['W2'],model['b2']
    W3,b3,W4,b4,W5,b5 = model['W3'],model['b3'],model['W4'],model['b4'],model['W5'],model['b5']
    N,C,H,W = X.shape
    
    # the convolve filter size of the two convolution layers are same
    
    assert W1.shape[2:] == W2.shape[2:], 'Conv filters must be the same size'
    conv_filter_height, conv_filter_width = W1.shape[2:]
    assert conv_filter_height == conv_filter_width, 'Conv filter must be square'
    assert conv_filter_height % 2 == 1, 'Conv filter height must be odd'
    assert conv_filter_width % 2 == 1, 'Conv filter width must be odd'
...
def init_five_layer_convnet(weight_scale=1e-3, bias_scale=0, input_shape=(3, 32, 32),
                 num_classes=10, hidden_size = 300, num_filters=32, filter_size=5):
    C,H,W = input_shape
    assert filter_size % 2 == 1, 'Filter size must be odd: got %d' % filter_size
    ...
    model['W4'] = weight_scale * np.random.randn(num_filters * H * W/16, hidden_size)
    model['b4'] = bias_scale * np.random.randn(hidden_size)
    model['W5'] = weight_scale * np.random.randn(hidden_size, num_classes)
